              ...
              <a-popover :title="`${column.title}-分析`" placement="bottomRight">
                  <template #content>
                      ...
                      <a-radio-group @change="numAnalys(column)" v-model:value="analysSource.value">
                          <a-radio style="display: block" value="num" >{{ column.title }}数量分布</a-radio>
                          <a-radio  style="display: block" v-for="numCol in genNumCols()" :value="numCol.dataIndex" :key="numCol.dataIndex">{{ column.title }}{{ numCol.title}}分布</a-radio>
                      </a-radio-group>
                      </p>
                  </template>
...
setup (props, ctx) {
  ...
  const numAnalys = (column) => {
    ...
    columnNumAnalys.value.column = column
    ...
    const groupByValues = groupBy(currentData.value, column.dataIndex)
    if (analysSource.value.value === 'num') {
      columnNumAnalys.value.title = `${columnNumAnalys.value.title}数量分布`
    } else {
      const addTitle = props.columns.find(x => x.dataIndex === analysSource.value.value).title
      columnNumAnalys.value.title = `${columnNumAnalys.value.title}${addTitle}分布`
    }
  ...
  const change = (pagination, filters, sorter, { currentDataSource }) => {
    currentData.value = currentDataSource
    if (columnNumAnalys.value !== undefined) {
      numAnalys(columnNumAnalys.value.column)
    }
    ctx.emit('change', pagination, filters, sorter, { currentDataSource })
  }
