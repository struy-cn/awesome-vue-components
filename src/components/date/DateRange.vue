<template>
  <a-range-picker ref="dateRangeRef" :allowClear="allowClear" :allowEmpty="allowEmpty"
      ...
      <template #renderExtraFooter>
          ...
          <a v-if="dateRange && dateRange.length == 2 && Number(someMonthNextMonth(dateRange,disabledAfterDate)[0].slice(5, 7)) <= currMonth"
              @click="chnageDateRange(someMonthNextMonth(dateRange,disabledAfterDate))">
              <a-tag color="success">{{ someMonthNextMonth(dateRange,disabledAfterDate)[0].slice(5, 7) }}月份</a-tag>
          </a>
